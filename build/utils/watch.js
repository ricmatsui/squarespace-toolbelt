...
var WatchExports = {
   ...
   */
  watchAndCollect: function watchAndCollect(_ref) {
    ...
    function handleAdd(filePath) {
      manager.getFiles(function (file) {
    ...
    function handleDelete(filePath) {
      var relPath = path.relative(manager.srcDir, filePath);
      var dest = manager.buildDir + relPath;
    ...
    var watcher = chokidar.watch(paths, {
      ignoreInitial: true,
      ignored: [path.join(manager.srcDir, '.DS_Store'), path.join(manager.srcDir, '**/.DS_Store'), manager.buildDir]
    });
   ...
   */
  watchFolder: function watchFolder(folder, excludePatterns, callback) {
    ...
    var watcher = chokidar.watch(path.resolve(folder), {
      ignored: excludePatterns.map(function (pattern) {
        return path.resolve(folder, pattern);
      }),
      ignoreInitial: true
    });
